...
#include "caffe/util/im2col_deeplab.hpp"
#include "caffe/util/math_functions.hpp"
#include "caffe/layers/conv_deeplab_layer.hpp"

namespace caffe {

template <typename Dtype>
void ConvolutionDeeplabLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  // Configure the kernel size, padding, stride, and inputs.
  ConvolutionDeeplabParameter conv_param = this->layer_param_.convolution_deeplab_param();
  ...
  num_output_ = this->layer_param_.convolution_deeplab_param().num_output();
  CHECK_GT(num_output_, 0);
  group_ = this->layer_param_.convolution_deeplab_param().group();
  ...
  bias_term_ = this->layer_param_.convolution_deeplab_param().bias_term();
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
        this->layer_param_.convolution_deeplab_param().weight_filler()));
    ...
    if (bias_term_) {
      ...
      shared_ptr<Filler<Dtype> > bias_filler(GetFiller<Dtype>(
          this->layer_param_.convolution_deeplab_param().bias_filler()));
...
template <typename Dtype>
void ConvolutionDeeplabLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  num_ = bottom[0]->num();
...
template <typename Dtype>
void ConvolutionDeeplabLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void ConvolutionDeeplabLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(ConvolutionDeeplabLayer);
#endif

INSTANTIATE_CLASS(ConvolutionDeeplabLayer);
REGISTER_LAYER_CLASS(ConvolutionDeeplab);
}  // namespace caffe
